...
// Attach click handler for the static place order button (fallback)
document.addEventListener('DOMContentLoaded', () => {
  const btn = document.getElementById('place-order-btn');
  if (btn) {
    btn.addEventListener('click', (e) => {
      // if the button is the one inside review step generated by fillReviewStep,
      // finalizeOrder will be called from that generated button. For the static
      // button we still call finalizeOrder to provide feedback.
      finalizeOrder();
    });
  }
});
